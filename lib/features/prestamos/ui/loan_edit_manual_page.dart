// lib/features/prestamos/ui/loan_edit_manual_page.dart
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:intl/intl.dart';
import '../loan_service.dart';

class LoanEditManualPage extends ConsumerStatefulWidget {
  final int prestamoId;
  const LoanEditManualPage({super.key, required this.prestamoId});

  @override
  ConsumerState<LoanEditManualPage> createState() => _LoanEditManualPageState();
}

class _LoanEditManualPageState extends ConsumerState<LoanEditManualPage> {
    ...
    try {
      ...
      if (data['plan_mode'] != 'manual') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Este préstamo no es manual; edición de plan no disponible.'))
        );
      ...
      _tasa.text = (data['tasa'] ?? 0).toString();
      ...
      try {
        _fechaInicio = DateTime.parse(fiStr);
      } catch (_) {}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Editar Préstamo (Manual)')),
      body: Form(
        key: _formKey,
        child: ListView(
            ...
            Row(children: [
              ...
              Expanded(
                child: TextFormField(
                  ...
                  readOnly: true, // por simplicidad; cambiarlo implica recalcular longitud/plan
